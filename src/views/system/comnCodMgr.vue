        ...
        <tbody>
          ...
          <template v-else>
            <tr v-for="item in items" :key="item.grp_cod">
              <td>{{ item.indexNew }}</td>
              <td id="groupTitle" @click="searchdetail(item.grp_cod, item.grp_cod_nm)">
                {{ item.grp_cod }}
              </td>
    ...
    <div>
      <p class="conTitle">
        <span>그룹 코드 상세</span>
          ...
          <a
            class="btn btn-primary mx-2"
            @click="rowDtlClicked(this.groupCode)"
            name="modal"
          >
            <span>신규등록</span>
          </a>
          ...
          <tbody>
            ...
            <template v-else>
              <tr v-for="item in groupDetail" :key="item.grp_cod">
                ...
                <td>{{ item.use_poa }}</td>
                <td>
                <a @click="rowDtlClicked(item.grp_cod, item.dtl_cod)" class="btnType3 color1"
                  ><span>수정</span></a
                >
              </td>
              </tr>
...
import comnGrpModal from "@/components/system/comnGrpModal.vue";
import comnGrpDetailModal from "@/components/system/comnGrpDetailModal.vue";
...
export default {
  ...
  methods: {
    searchdetail: function (group_code, group_code_name) {
      let vm = this;

      vm.groupCode = group_code; // for 페이징 넘버 클릭시를 위함
      vm.groupCodeName = group_code_name; // for 페이징 넘버 클릭시를 위함
    ...
    rowClicked: async function (grpcd) {
      if (grpcd == null || grpcd == "") {
        ...
        const modal = await openModal(comnGrpModal, {
          title: "그룹코드 등록",
          grpcd: "",
          action: this.action,
        });
    ...
    rowDtlClicked: async function (grpcd, dtlcod) {
      ...
      if (dtlcod == null || dtlcod == "") {
        ...
        const modal = await openModal(comnGrpDetailModal, {
          title: "상세코드 관리",
